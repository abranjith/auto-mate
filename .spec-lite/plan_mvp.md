...
| FEAT-111 | **Save-and-rerun** — save a completed task and run it again against another file, with input compatibility checks and recorded rules, inputs, parameters, and environment. Basic reuse only; the richer gallery is deferred per D02. | D11 (templates with immutable revisions, recorded inputs/parameters/runtime, explicit mapping and repair creating a revision for approval, historical replay) — **resolved for this row on 2026-09-25; see the spec's "Decisions applied" table** | `features/FEAT-111-save_and_rerun/spec.md` | [ ] Not started |
